...
void CassandraClient::insertFailed()
{
    ...
    for (const auto& obj : actionTraces)
    {
        ...
        insertActionTrace(obj.globalSeq, std::move(s), actionType, receiver, account);
    }
...
void CassandraClient::prepareStatements()
{
    ...
    std::string insertActionTraceQuery = "INSERT INTO " + action_trace_table +
        " (part_key, global_seq, doc, action_type, receiver, account) VALUES(?, ?, ?, ?, ?, ?)";
    ...
    const std::string& account)
{
    ...
    CassStatement* statement = cass_prepared_bind(gPreparedInsertActionTrace_.get());
    auto gStatement = statement_guard(statement, cass_statement_free);
    auto globalSeqBuf = num_to_bytes(globalSeq);
    cass_statement_bind_int32_by_name(statement, "part_key", globalSeq / 100000000);
    cass_statement_bind_bytes_by_name(statement, "global_seq", globalSeqBuf.data(), globalSeqBuf.size());
